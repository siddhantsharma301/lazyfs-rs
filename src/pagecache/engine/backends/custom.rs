...
pub struct CustomCacheEngine {
    ...
    lru_management_mtx: Mutex<()>,
}

impl CustomCacheEngine {
    fn new(config: Box<Config>) -> Self {
        CustomCacheEngine {
            lock_cache_mtx: RwLock::new(()),
            config,
            ...
            lru_management_mtx: Mutex::new(()),
        }
    }

    fn get_page_ptr(&self, page_id: i32) -> Option<Box<Page>> {
        self.search_index.get(&page_id).cloned()
    }

    fn has_empty_pages(&self) -> bool {
        self.free_pages.len() > 0
    }
    ...
    fn get_next_free_page(&mut self, owner_id: String) -> Result<(i32, Option<Box<Page>>)> {
        ...
        if let Some(free_pages) = self.owner_free_pages_mapping.get_mut(&owner_id) {
        ...
        if let Some(&last_index) = self.free_pages.last() {
            self.free_pages.pop();
        ...
        if self.config.apply_lru_eviction {
            let replace_place_id = match self.lru_main_vector.back() {
                Some(r) => *r,
            ...
            for block_id in blocks.keys() {
                self.owner_ordered_pages_mapping
                    .get_mut(&old_owner)
                    .and_then(|pages| pages.remove(&block_id));
            }
            self.owner_free_pages_mapping
                .get_mut(&old_owner)
    ...
    fn apply_lru_after_page_visitation_on_write(&mut self, visited_page_id: i32) {
        let _lock = self.lru_management_mtx.lock().unwrap();
        if let Some(&position) = self.page_order_mapping.get(&visited_page_id) {
            self.lru_main_vector.remove(position as usize);
        }

        self.lru_main_vector.push_front(visited_page_id);
        let front_position = self.lru_main_vector.front().unwrap();
        self.page_order_mapping
            .insert(visited_page_id, *front_position);

        // If the LRU list is larger than the cache size, remove the least recently used page
        if self.page_order_mapping.len() > self.config.cache_nr_pages as usize {
            if let Some(&back_page_id) = self.lru_main_vector.back() {
                self.page_order_mapping.remove(&back_page_id);
                self.lru_main_vector.pop_back();
            }
        }
    }
    ...
    fn apply_lru_after_page_visitation_on_read(&mut self, visited_page_id: i32) {
        ...
        self.lru_main_vector.push_front(visited_page_id);
        let new_position = self.lru_main_vector.front().unwrap();
        self.page_order_mapping
    ...
    fn update_owner_pages(
        ...
        if real_owner == "none" || real_owner != new_owner {
            ...
            if self.owner_pages_mapping.contains_key(&real_owner) {
                self.owner_pages_mapping
                    .get_mut(&real_owner)
                    .unwrap()
                    .remove(&page_id);

                if self.owner_ordered_pages_mapping.contains_key(&real_owner) {
                    self.owner_ordered_pages_mapping
                        .get_mut(&real_owner)
                        .unwrap()
                        .remove(&block_id);
                ...
                if self.owner_pages_mapping.get(&real_owner).unwrap().is_empty() {
                    self.owner_pages_mapping.remove(&real_owner);
                    self.owner_free_pages_mapping.remove(&real_owner);
                    self.owner_ordered_pages_mapping.remove(&real_owner);
                }
            }
        }
    ...
    //         let _lock = self.lock_cache_mtx.write().unwrap();
    ...
    //         let (free_page_id, free_page_ptr) = self.get_next_free_page(content_owner_id.clone());
    //         if free_page_id >= 0 {
    //             if let Some(page) = free_page_ptr {
    ...
    //                 self.update_owner_pages(content_owner_id, free_page_id, blk_id, offs);
    //             } else {
    ...
    //     for (&blk_id, &(page_id, ref data, read_to_max_index)) in &block_pages {
    //         let _lock = self.lock_cache_mtx.read().unwrap();
    ...
    pub fn get_engine_usage(&self) -> f64 {
        let _lock = self.lock_cache_mtx.read().unwrap();
        let used_pages = self.config.cache_nr_pages - self.free_pages.len();
    ...
    //     self.owner_free_pages_mapping.remove(&owner);

    //     // Process each page owned by the owner
    //     if let Some(owner_pgs) = self.owner_pages_mapping.remove(&owner) {
    ...
    //             if self.config.apply_lru_eviction {
    //                 if let Some(position) = self.page_order_mapping.remove(&page_id) {
    //                     self.lru_main_vector.remove(position as usize);
    //                 }
    //             }

    //             // Reset the page and change its owner to "none"
    //             if let Some(page_ptr) = self.get_page_ptr(page_id) {
    //                 page_ptr.reset();
    //                 page_ptr.change_owner("none".to_string());
    //             }
    //         }
    //         self.owner_ordered_pages_mapping.remove(&owner);
    //     }
    ...
    //     if let Some(iterate_blocks) = self.owner_ordered_pages_mapping.get_mut(&owner) {
    ...
    //         for (index, (block_id, (page_id, page, offsets, flag))) in
    //             iterate_blocks.iter_mut().enumerate()
    //         {
    //             if page.is_page_dirty() {
    //                 new_iterate_blocks.insert(index as i32, (*page_id, **page, *offsets, *flag));
    ...
    //                     let streak_pair = &page_chunk[p_id as usize];
    ...
    //     if !self.owner_pages_mapping.contains_key(&old_owner) {
    //         return false;
    //     }

    //     // Retrieve the old owner's data
    //     let old_page_mapping = self.owner_pages_mapping.remove(&old_owner).unwrap();
    //     let old_free_mapping = self.owner_free_pages_mapping.remove(&old_owner).unwrap();
    //     let old_ordered_pages = self.owner_ordered_pages_mapping.remove(&old_owner).unwrap();
    ...
    //                         self.owner_pages_mapping.get_mut(&content_owner_id)
    //                     {
    //                         owner_pages.remove(&page_id);
    //                     }
    //                     if let Some(ordered_pages) =
    //                         self.owner_ordered_pages_mapping.get_mut(&content_owner_id)
    //                     {
    ...
    pub fn get_dirty_blocks_info(&self, owner: String) -> Vec<(i32, (i32, i32), i32)> {
        ...
        if let Some(ordered_pages) = self.owner_ordered_pages_mapping.get(&owner) {
